...
_TF_RANDOM_DTYPES = [tf.float16, tf.bfloat16, tf.float32, tf.float64, tf.int32, tf.int64]

_TF_COMPLEX_DTYPES = [tf.complex64, tf.complex128]
    ...
    if dtype is None:
      ...
      if dtype == tf.complex64:
        return tf.complex(tf.random.uniform(shape=shape, minval=min_val, maxval=max_val, dtype=tf.float32, seed=seed),
                          tf.random.uniform(shape=shape, minval=min_val, maxval=max_val, dtype=tf.float32, seed=seed))
      else:
