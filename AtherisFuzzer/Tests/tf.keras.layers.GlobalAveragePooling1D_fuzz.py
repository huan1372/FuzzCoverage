#This is a Python API fuzzer for tf.keras.layers.GlobalAveragePooling1D
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.GlobalAveragePooling1D_exception.txt","a")
	try:
		name_choices = []
		name_STR_strlist = ['global_average_pooling1d_1', 'global_average_pooling1d'] 
		...
		arg_class = tf.keras.layers.GlobalAveragePooling1D(name=name,trainable=trainable,batch_input_shape=batch_input_shape,dtype=dtype,data_format=data_format)
