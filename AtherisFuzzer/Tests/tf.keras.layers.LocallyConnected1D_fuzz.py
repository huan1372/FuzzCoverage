#This is a Python API fuzzer for tf.keras.layers.LocallyConnected1D
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.LocallyConnected1D_exception.txt","a")
	try:
		filters_choices = []
		...
		name_choices = []
		name_STR_strlist = ['locally_connected1d'] 
		...
		arg_class = tf.keras.layers.LocallyConnected1D(filters=filters,kernel_size=kernel_size,padding=padding,strides=strides,data_format=data_format,implementation=implementation,name=name,trainable=trainable,dtype=dtype,activation=activation,use_bias=use_bias,kernel_initializer=kernel_initializer,bias_initializer=bias_initializer,kernel_regularizer=kernel_regularizer,bias_regularizer=bias_regularizer,activity_regularizer=activity_regularizer,kernel_constraint=kernel_constraint,bias_constraint=bias_constraint,batch_input_shape=batch_input_shape)
