#This is a Python API fuzzer for tf.tensor_scatter_nd_update
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.tensor_scatter_nd_update_exception.txt","a")
	try:
		parameter_0_choices = []
		parameter_0_LIST = fh.get_int_list(min_length=3, max_length=8)
		...
		parameter_1_DTYPES = [tf.int32,tf.int64]
		int_list = fh.get_int_list(min_length=2,max_length=2)
		min_Val = min(int_list) - 1
		max_Val = max(int_list)
		if min_Val % 2 == 0:
		...
		parameter_2_choices = []
		parameter_2_LIST = fh.get_int_list(min_length=2, max_length=4)
		...
		arg_class = tf.tensor_scatter_nd_update(parameter_0,parameter_1,parameter_2)
