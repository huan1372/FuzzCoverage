#This is a Python API fuzzer for tf.keras.layers.Conv3D
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.Conv3D_exception.txt","a")
	try:
		parameter_0_choices = []
		...
		kernel_size_choices = []
		kernel_size_LIST = fh.get_int_list(min_length=3, max_length=3)
		...
		dilation_rate_LIST = fh.get_int_list(min_length=3, max_length=3)
		...
		name_choices = []
		name_STR_strlist = ['conv3d_transpose'] 
		...
		arg_class = tf.keras.layers.Conv3D(parameter_0,parameter_1,activation=activation,input_shape=input_shape,filters=filters,kernel_size=kernel_size,strides=strides,padding=padding,data_format=data_format,dilation_rate=dilation_rate,use_bias=use_bias,kernel_initializer=kernel_initializer,bias_initializer=bias_initializer,kernel_regularizer=kernel_regularizer,bias_regularizer=bias_regularizer,activity_regularizer=activity_regularizer,kernel_constraint=kernel_constraint,bias_constraint=bias_constraint,name=name,trainable=trainable,batch_input_shape=batch_input_shape,dtype=dtype,groups=groups)
