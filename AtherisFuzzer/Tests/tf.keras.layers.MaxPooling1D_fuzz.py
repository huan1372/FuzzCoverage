#This is a Python API fuzzer for tf.keras.layers.MaxPooling1D
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.MaxPooling1D_exception.txt","a")
	try:
		pool_size_choices = []
		pool_size_INT = fh.get_int()
		pool_size_choices.append(pool_size_INT)
		pool_size_LIST = fh.get_int_list(min_length=1, max_length=1)
		pool_size_choices.append(pool_size_LIST)
		pool_size = pool_size_choices[fh.get_int()%2]
		strides_choices = []
		...
		arg_class = tf.keras.layers.MaxPooling1D(pool_size=pool_size,strides=strides,padding=padding,name=name,trainable=trainable,dtype=dtype,data_format=data_format,batch_input_shape=batch_input_shape)
