#This is a Python API fuzzer for tf.keras.layers.RepeatVector
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.RepeatVector_exception.txt","a")
	try:
		n_choices = []
		n_INT = fh.get_int()
		n_choices.append(n_INT)
		n = n_choices[0]
		name_choices = []
		name_STR_strlist = ['repeat_vector'] 
		...
		batch_input_shape_LIST = fh.get_int_list(min_length=2, max_length=2)
		...
		arg_class = tf.keras.layers.RepeatVector(n=n,name=name,trainable=trainable,batch_input_shape=batch_input_shape,dtype=dtype,parameter_0)
