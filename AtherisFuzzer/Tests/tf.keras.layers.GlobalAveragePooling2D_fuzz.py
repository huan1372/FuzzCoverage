#This is a Python API fuzzer for tf.keras.layers.GlobalAveragePooling2D
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.GlobalAveragePooling2D_exception.txt","a")
	try:
		name_choices = []
		name_STR_strlist = ['global_average_pooling2d_1', 'global_average_pooling2d'] 
		...
		arg_class = tf.keras.layers.GlobalAveragePooling2D(name=name,trainable=trainable,dtype=dtype,data_format=data_format,batch_input_shape=batch_input_shape)
