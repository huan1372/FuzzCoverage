#This is a Python API fuzzer for tf.keras.layers.AlphaDropout
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.AlphaDropout_exception.txt","a")
	try:
		rate_choices = []
		...
		rate = rate_choices[0]
		name_choices = []
		name_STR_strlist = ['alpha_dropout'] 
		...
		arg_class = tf.keras.layers.AlphaDropout(rate=rate,name=name,trainable=trainable,dtype=dtype,batch_input_shape=batch_input_shape,parameter_0)
