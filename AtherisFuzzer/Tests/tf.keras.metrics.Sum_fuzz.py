#This is a Python API fuzzer for tf.keras.metrics.Sum
import atheris
...
def TestOneInput(data):
	...
	try:
		dtype_choices = []
		dtype_DTYPE_dtypelist = ['', 'same', 'valid', 'sum', 'tf.float64', 'zeros', '1'] 
		...
		input_signature_LIST_intLlist = [[1, 2], [[[1.0, 2.0], [3.0, 2.0], [0.5, 4.0]]], [1, 5], [[1], [5]]] 
		...
		input_signature_INT_intlist = [1000, 100, 300, 200] 
		...
		name_choices = []
		name_STR_strlist = ['', 'same', 'valid', 'sum', 'my_sum', 'zeros', '1'] 
		name_STR = name_STR_strlist[fh.get_int(min_int=0, max_int=len(name_STR_strlist)-1)]
		name_choices.append(name_STR)
		name = name_choices[0]
		arg_class = tf.keras.metrics.Sum(dtype=dtype,name=name)
