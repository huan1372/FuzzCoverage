#This is a Python API fuzzer for tf.nn.softmax
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.nn.softmax_exception.txt","a")
	try:
		parameter_0_choices = []
		...
		logits_choices = []
		# Tensor generation for logits
		logits_DTYPES = [tf.float32]
		int_list = fh.get_int_list(min_length=2,max_length=2)
		min_Val = min(int_list) - 1
		max_Val = max(int_list)
		if min_Val % 2 == 0:
			logits_tensor = fh.get_random_numeric_tensor(dtype=fh.get_tf_dtype(allowed_set=logits_DTYPES))
		else:
			logits_tensor = fh.get_random_numeric_tensor(min_val = min_Val, max_val = max_Val, dtype=fh.get_tf_dtype(allowed_set=logits_DTYPES))
		logits_tensor = tf.identity(logits_tensor)
		logits_choices.append(logits_tensor)
		logits = logits_choices[0]
		axis_choices = []
		axis_INT = fh.get_int()
		axis_choices.append(axis_INT)
		axis = axis_choices[0]
		arg_class = tf.nn.softmax(parameter_0,logits=logits,axis=axis)
