#This is a Python API fuzzer for tf.keras.layers.Permute
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.Permute_exception.txt","a")
	try:
		name_choices = []
		name_STR_strlist = ['permute'] 
		...
		dims_choices = []
		dims_LIST = fh.get_int_list(min_length=2, max_length=2)
		dims_choices.append(dims_LIST)
		dims = dims_choices[0]
		...
		arg_class = tf.keras.layers.Permute(name=name,trainable=trainable,dtype=dtype,dims=dims,batch_input_shape=batch_input_shape)
