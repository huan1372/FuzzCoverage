#This is a Python API fuzzer for tf.keras.layers.Conv2D
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.Conv2D_exception.txt","a")
	try:
		filters_choices = []
		...
		kernel_initializer_TF_OBJECT_tfobjlist = ['Ones', 'GlorotUniform'] 
		...
		arg_class = tf.keras.layers.Conv2D(filters=filters,kernel_size=kernel_size,parameter_0,parameter_1,activation=activation,input_shape=input_shape,dilation_rate=dilation_rate,padding=padding,parameter_2,strides=strides,data_format=data_format,use_bias=use_bias,kernel_initializer=kernel_initializer,bias_initializer=bias_initializer,kernel_regularizer=kernel_regularizer,bias_regularizer=bias_regularizer,activity_regularizer=activity_regularizer,kernel_constraint=kernel_constraint,bias_constraint=bias_constraint,name=name,trainable=trainable,dtype=dtype,groups=groups,batch_input_shape=batch_input_shape)
