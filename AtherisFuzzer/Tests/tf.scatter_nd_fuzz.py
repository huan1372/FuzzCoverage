#This is a Python API fuzzer for tf.scatter_nd
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.scatter_nd_exception.txt","a")
	try:
		parameter_0_choices = []
		...
		parameter_1_DTYPES = [tf.bool,tf.float32]
		int_list = fh.get_int_list(min_length=2,max_length=2)
		min_Val = min(int_list) - 1
		max_Val = max(int_list)
		if min_Val % 2 == 0:
		...
		parameter_2_choices = []
		parameter_2_LIST = fh.get_int_list(min_length=2, max_length=2)
		parameter_2_choices.append(parameter_2_LIST)
		parameter_2 = parameter_2_choices[0]
		shape_choices = []
		shape_LIST = fh.get_int_list(min_length=3, max_length=4)
		shape_choices.append(shape_LIST)
		shape = shape_choices[0]
		arg_class = tf.scatter_nd(parameter_0,parameter_1,parameter_2,shape=shape)
