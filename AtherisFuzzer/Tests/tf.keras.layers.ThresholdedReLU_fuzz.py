#This is a Python API fuzzer for tf.keras.layers.ThresholdedReLU
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.ThresholdedReLU_exception.txt","a")
	try:
		name_choices = []
		name_STR_strlist = ['thresholded_re_lu'] 
		...
		theta_choices = []
		theta_FLOAT = fh.get_float()
		theta_choices.append(theta_FLOAT)
		theta = theta_choices[0]
		...
		arg_class = tf.keras.layers.ThresholdedReLU(name=name,trainable=trainable,dtype=dtype,theta=theta,batch_input_shape=batch_input_shape)
