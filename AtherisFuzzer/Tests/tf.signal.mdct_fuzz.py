#This is a Python API fuzzer for tf.signal.mdct
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.signal.mdct_exception.txt","a")
	try:
		parameter_0_choices = []
		...
		pad_end_choices = []
		pad_end_BOOL = fh.get_bool()
		pad_end_choices.append(pad_end_BOOL)
		pad_end = pad_end_choices[0]
		window_fn_choices = []
		window_fn = window_fn_choices[fh.get_int()%0]
		arg_class = tf.signal.mdct(parameter_0,parameter_1,pad_end=pad_end,window_fn=window_fn)
