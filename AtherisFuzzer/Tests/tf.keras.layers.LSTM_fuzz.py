#This is a Python API fuzzer for tf.keras.layers.LSTM
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.LSTM_exception.txt","a")
	try:
		parameter_0_choices = []
		...
		return_runtime_choices = []
		return_runtime_BOOL = fh.get_bool()
		return_runtime_choices.append(return_runtime_BOOL)
		return_runtime = return_runtime_choices[0]
		name_choices = []
		name_STR_strlist = ['lstm', 'lstm_1'] 
		...
		recurrent_activation_STR_strlist = ['hard_sigmoid', 'sigmoid'] 
		...
		arg_class = tf.keras.layers.LSTM(parameter_0,return_sequences=return_sequences,return_state=return_state,return_runtime=return_runtime,name=name,trainable=trainable,dtype=dtype,go_backwards=go_backwards,stateful=stateful,unroll=unroll,time_major=time_major,units=units,activation=activation,recurrent_activation=recurrent_activation,use_bias=use_bias,kernel_initializer=kernel_initializer,recurrent_initializer=recurrent_initializer,bias_initializer=bias_initializer,unit_forget_bias=unit_forget_bias,kernel_regularizer=kernel_regularizer,recurrent_regularizer=recurrent_regularizer,bias_regularizer=bias_regularizer,activity_regularizer=activity_regularizer,kernel_constraint=kernel_constraint,recurrent_constraint=recurrent_constraint,bias_constraint=bias_constraint,dropout=dropout,recurrent_dropout=recurrent_dropout,implementation=implementation,batch_input_shape=batch_input_shape)
