#This is a Python API fuzzer for tf.keras.layers.MaxPooling2D
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.MaxPooling2D_exception.txt","a")
	try:
		pool_size_choices = []
		...
		name_choices = []
		name_STR_strlist = ['max_pooling2d', 'max_pooling2d_1'] 
		...
		arg_class = tf.keras.layers.MaxPooling2D(pool_size=pool_size,input_shape=input_shape,strides=strides,padding=padding,parameter_0,name=name,trainable=trainable,dtype=dtype,data_format=data_format,batch_input_shape=batch_input_shape)
