#This is a Python API fuzzer for tf.keras.layers.SpatialDropout3D
import atheris
...
def TestOneInput(data):
	...
	f = open("/home/usr/FreeFuzz/FuzzCoverage/AtherisFuzzer/Exceptions/tf.keras.layers.SpatialDropout3D_exception.txt","a")
	try:
		name_choices = []
		name_STR_strlist = ['spatial_dropout3d_1', 'spatial_dropout3d'] 
		...
		arg_class = tf.keras.layers.SpatialDropout3D(name=name,trainable=trainable,dtype=dtype,rate=rate,noise_shape=noise_shape,seed=seed,batch_input_shape=batch_input_shape,data_format=data_format)
